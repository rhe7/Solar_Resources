function [P W act num_of_members] = stochastic_load_model(S, A, L)

% STOCHASTIC_LOAD_MODEL (updated by Joakim Widén 180116)
%
...
%        number of persons. The states are numbered 1-12, corresponding
%        to the activities:
%
%        1 away
%        2 sleeping
...
act = synth_act_data_general(number_of_persons, days, M_wd, M_wed);
...
% Lighting
P.lighting = lighting_load(act, L_subset, num_of_members, lighting_param);

% Additional
P.add = num_of_members' * ones(1,INTERVALS) * A.ADDITIONAL;

% Total load
P.total = P.cold_appliances + P.cooking + P.washing + P.dishwashing + P.tv + P.computer + P.audio + P.lighting + P.add;
...
% Total DHW
W.total = W.bathing + W.showering + W.additional;
